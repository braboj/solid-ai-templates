...
An audit is triggered on demand, by an event that changes what the
review reads:
...
A release does not trigger one by itself. Where a project gates its
release on a current audit, the obligation is paced by the age of the
newest record against the review interval declared with that gate: a
release whose newest record falls inside the interval owes neither the
audit nor a record declining it, whatever version it moves.
